...
## Procedure
Though in both American and Polish validation studies the parents were asked to fill in both the full CDI and the CAT-CDI, the procedure differed between the two studies. In the validation of American English CDI-CAT, parents were given both CDIs in one sitting, but the order of the CDI versions was counterbalanced. In the Polish validation study, the order of the CDIs was fixed, with the full CDI always first, and then, after 1–30 days (*M* = `r round(mean(by_child_pl$days_between),2)`, *Mdn* = `r round(median(by_child_pl$days_between),2)`, SD = `r round(sd(by_child_pl$days_between),2)`) parents were asked to fill in the CAT-CDI. Both studies were done fully online and obtained the approval of adequate ethics committees.
...
We found similarly strong correlations in the two languages between the abilities estimated from CDI-CAT and full CDI scores (English and Polish: $r$ = .86). The correlations were also strong within individual age groups (see Tables \@ref(tab:corr-by-age-eng) and \@ref(tab:corr-by-age-pl)). We also obtained IRT ability estimates from the full CDI using the same 2-PL model. Because these estimates draw on responses to all available items in the full CDI, they incorporate more information about each child’s performance, reducing random error and increasing precision. The 2-PL model further adjusts for item difficulty and discrimination, making these ability scores robust and comparable across individuals. Such ability from the full test version is often considered a baseline for comparisons with ability estimated from a short (adaptive) version. We correlated the abilities estimated from the CDI-CAT and abilities estimated from full CDI and again found strong correlations in both languages, English and Polish: $r$ = .92. Finally, we also correlated the abilities estimated from the full CDI and the full CDI scores, i.e. two measures obtained from the same data. This way we examine consistency between model-based abilities and traditional scores. Again, we found strong correlations in both languages (English: $r$ = .95, Polish: $r$ = `r cor(by_child_pl$full_theta, by_child_pl$score_full)`), confirming that both measures reflect the same underlying ability. 
...
Next, we looked at the mean squared error (MSE) which quantifies the differences between the abilities as estimated by CAT-CDI and from the full CDI (the differences are squared to eliminate negative values). The mean squared error in English was `r mean(prod_s$sq_err)` (*Mdn* = `r median(prod_s$sq_err)`, *SD* = `r sd(prod_s$sq_err)`), and in Polish it was `r mean(by_child_pl$sq_err)` (*Mdn* = `r median(by_child_pl$sq_err)`, *SD* = `r sd(by_child_pl$sq_err)`). The mean squared error was relatively high in English (compared to Polish), suggesting a relatively larger discrepancy between CAT-CDI ability estimates and full CDI ability estimates in English. This occurred despite strong correlations between the two measures. This difference between the full and CAT-CDI estimates may be explained by at least two reasons. First, a number of participants with extreme discrepancies between the CAT-CDI and full CDI estimates may be inflating the MSE. Second, there might be a systematic bias, with one estimate consistently higher than the other. 
...
To explore the first of these possibilities, we identified children for whom the estimates from the CAT-CDI and full CDI showed extreme discrepancies, i.e. the difference between the two estimates was removed by 1.5 SD from the mean (measured in MSE). There were `r sum(prod_s$extreme_discrep == "yes")` such cases (`r round(sum(prod_s$extreme_discrep == "yes")/count(prod_s)*100,2)`%) in the English dataset and `r sum(by_child_pl$extreme_discrep == "yes")` cases (`r round(sum(by_child_pl$extreme_discrep == "yes")/count(prod_s)*100,2)`%) in the Polish dataset. We then re-calculated the mean squared error without these cases, which yielded an MSE of `r round(prod_s %>% filter(extreme_discrep == "no") %>% summarise(mean(sq_err)),2)` (*Mdn* = `r round(prod_s %>% filter(extreme_discrep == "no") %>% summarise(median(sq_err)),2)`, *SD* = `r round(prod_s %>% filter(extreme_discrep == "no") %>% summarise(sd(sq_err)),2)`) in English and `r round(by_child_pl %>% filter(extreme_discrep == "no") %>% summarise(mean(sq_err)),2)` (*Mdn* = `r round(by_child_pl %>% filter(extreme_discrep == "no") %>% summarise(median(sq_err)),2)`, *SD* = `r round(by_child_pl %>% filter(extreme_discrep == "no") %>% summarise(sd(sq_err)),2)`) in Polish. The slightly reduced MSE values indicate that cases of extreme discrepancy had only a modest impact on the overall error and do not fully explain the differences observed between ability estimates from full CDI and CAT-CDI in the two languages.
...
Next, we examined a possible bias in our two ability estimates. We found that the mean CAT ability estimates were consistently higher than the full CDI ability estimates. That was true both in English, `r t1$full_result` (large effect size: Cohen's $d$ = 1.06, 95% CI $[0.89, 1.23]$), and in Polish, `r t2$full_result` (medium effect size: Cohen's $d$ = 0.73, 95% CI $[0.52, 0.93]$).

```{r}
# Hypo1: CDI full underestimates true ability
# The larger child's vocab size (full theta), the larger the difference between full and CAT (possibly due to parents failing to mark words in full CDI).

# EN
c1 <- apa_print(cor.test(prod_s$fullTheta, prod_s$full_cat_diff, paired = T))

# full - CAT
prod_s %>% filter(extreme_discrep == "no") %>% 
     ggplot(aes(x = fullTheta, y = (full_cat_diff))) + 
                 geom_point() + 
                 geom_smooth(method = "lm") + 
          geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
     ylim(-2,2) + xlim(-4,4) + 
     labs(title = "ENGLISH", y = "full - CAT")

# PL
c2 <- apa_print(cor.test(by_child_pl$full_theta, by_child_pl$full_cat_diff, paired = T))

# full - CAT
by_child_pl %>% filter(extreme_discrep == "no") %>% 
     ggplot(aes(x = full_theta, y = (full_cat_diff))) + 
                 geom_point() + 
                 geom_smooth(method = "lm") + 
     geom_hline(yintercept = 0, color = "red", linetype = "dashed") +
     ylim(-2, 2) + xlim(-4,4) + 
...
```{r}
# Theta by SE - comparing CAT and full

# EN
prod_s %>%
  ggplot() +
  geom_point(aes(x = fullTheta, y = fullTheta_SE)) +
  geom_point(aes(x = catTheta, y = catTheta_SE), color = "red") +
  geom_segment(aes(x = fullTheta, y = fullTheta_SE, xend = catTheta, yend = catTheta_SE, group = subject_id), color = "black") + labs(y = "SE", x = "Theta", title = "(English)") + ylim(0, 0.5)


prod_s %>%
  ggplot() +
  geom_smooth(aes(x = fullTheta, y = fullTheta_SE), se = T, method = "loess", span = 0.8, color = "black") +
  geom_smooth(aes(x = catTheta, y = catTheta_SE), se = T, method = "loess", span = 0.8, color = "red") +
  labs(y = "SE", x = "Theta", title = "(English)") + ylim(0, 0.5)

# PL
by_child_pl %>%
  ggplot() +
  geom_point(aes(x = full_theta, y = full_theta_se)) +
  geom_point(aes(x = cat_theta, y = cat_theta_se), color = "red") +
  geom_segment(aes(x = full_theta, y = full_theta_se, xend = cat_theta, yend = cat_theta_se, group = id), color = "black") + labs(y = "SE", x = "Theta", title = "(Polish)") + ylim(0, 0.5)

by_child_pl %>%
  ggplot() +
  geom_smooth(aes(x = full_theta, y = full_theta_se), se = T, method = "loess", span = 1, color = "black") +
  geom_smooth(aes(x = cat_theta, y = cat_theta_se), se = T, method = "loess", span = 1 , color = "red") +
  labs(y = "SE", x = "Theta", title = "(Polish)") + ylim(0, 0.5)


by_child_pl <- by_child_pl %>%
  mutate(high_se = full_theta_se > cat_theta_se)
by_child_pl %>%
  ggplot() +
  # full_theta points: black if highlighted, grey if not
  geom_point(aes(x = full_theta, y = full_theta_se, color = high_se), show.legend = FALSE) +
  # cat_theta points: red if highlighted, pink if not
  geom_point(data = filter(by_child_pl, high_se == TRUE), aes(x = cat_theta, y = cat_theta_se), color = "red") +
  geom_point(data = filter(by_child_pl, high_se == FALSE), aes(x = cat_theta, y = cat_theta_se), color = "pink") +
  # segments: black if highlighted, grey otherwise
  geom_segment(aes(
    x = full_theta, y = full_theta_se,
    xend = cat_theta, yend = cat_theta_se,
    group = id,
    color = high_se
  ), show.legend = FALSE) +
  scale_color_manual(
    values = c("TRUE" = "black", "FALSE" = "grey70")
  ) + labs(y = "SE", x = "Theta", title = "Cases when SE in CAT was lower than SE in full (Polish)") + ylim(0, 0.5)


by_child_pl <- by_child_pl %>%
  mutate(
    theta_diff = abs(full_theta - cat_theta),
    high_se = rank(-theta_diff) <= 10   # top 10 highest theta differences
  )
...
by_child_pl %>%
  ...
  geom_point(aes(x = full_theta, y = full_theta_se, color = high_se), show.legend = FALSE) +
  # cat_theta points: red if highlighted, pink if not
  geom_point(data = filter(by_child_pl, !high_se), aes(x = cat_theta, y = cat_theta_se), color = "pink") +
  geom_point(data = filter(by_child_pl, high_se), aes(x = cat_theta, y = cat_theta_se), color = "red") +
  # segments: black if highlighted, grey otherwise
  geom_segment(aes(
    x = full_theta, y = full_theta_se,
    xend = cat_theta, yend = cat_theta_se,
  ...
  ) + labs(y = "SE", x = "Theta", title = "Biggest differences between thetas (Polish)") + ylim(0, 0.5)
...
The question that remains is whether this consistent difference between the two estimates of abilities comes from CAT over-estimating child's lexical ability or from the full CDI underestimating child's ability. For one, it may be that in CAT-CDI parents answer "yes, my child produces this word" to more items that would be expected (Kachergis et al. 2022), and as a result, CAT-CDI may overestimate the child's true ability. On the other hand, it could be that as the full CDI is very long, parents may omit marking some items that their child produces (see e.g., Łuniewska et al., 2025 for parental inconsistencies in reporting vocabulary of older children). This could result in the full CDI underestimating the child's true ability. It is possible that either or both are true and the question is difficult to answer without direct assessment of child's lexical ability. However, we will attempt to tentatively address this question here. If parents are prone to omitting items in the full CDI, this would be most visible in the case of children with large vocabularies (the more words the child knows, the more CDI items can be omitted). If that is the case, then we may expect to see that as a child's ability grows (as estimated from the full CDI), the size of the discrepancy between the ability estimate from full CDI and the CAT CDI grows as well. English: `r c1$full_result`, Polish: `r c2$full_result`.
...
```{r}
# Hypo 2: CAT overestimates child's true ability
# Are parents more likely to respond “yes” when the item is asked in CAT compared to when it’s asked in the full CDI?
...
# # EN
# prod_s %>% filter(extreme_discrep == "no") %>% 
#      ggplot(aes(x = n_asked, y = (full_cat_diff))) + 
#                  geom_point() + 
#                  geom_smooth(method = "lm")
# 
# # PL
# by_child_pl %>% filter(extreme_discrep == "no") %>% 
#      ggplot(aes(x = no_items, y = (full_cat_diff))) + 
#                  geom_point() + 
#                  geom_smooth(method = "lm")
# 
# # PL
# by_child_pl %>% filter(extreme_discrep == "no") %>% 
#      ggplot(aes(x = full_theta, y = abs(sq_err))) + geom_point() + geom_smooth(method = "loess") + 
#      labs(title = "POLISH", y = "Squared Error (CAT vs full ability) (abs)")




# which words get "yes" in CAT and are not marked in full?
# I call them flips

consistency_pl %>% filter(answer_cat == 1 & answer_full == 0) %>%
     group_by(lexical_class) %>% count()

n_admin <- consistency_pl %>%
     group_by(definition) %>% count() %>% rename(n_admin = n)

n_flips <- consistency_pl %>% filter(answer_cat == 1 & answer_full == 0) %>%
     group_by(definition) %>% count() %>% rename(n_flipped = n)

flips_pl <- merge(n_admin, n_flips, by = "definition", all.x = T)
rm(n_admin, n_flips)
flips_pl[is.na(flips_pl)] <- 0
flips_pl <- flips_pl %>% rename(item = definition)
flips_pl <- merge(flips_pl, items_cat_pl)

# which items are flipped more frequently than others?
flips_pl %>% ggplot(aes(x = d, y = n_flipped)) + geom_jitter(alpha = 0.3) + 
     labs(title = "YES in CAT & NO in full", y = "count")
flips_pl %>% ggplot(aes(x = itemID, y = n_flipped)) + geom_jitter(alpha = 0.3)
# and half of administered are verbs, half of flipped are verbs
...
# FLIPS BY ID
# do some parents flip more or is it similar across parents?

flip_parents <- consistency_pl %>% filter(answer_cat == 1 & answer_full == 0) %>%
     group_by(id) %>% count()
flip_parents <- merge(flip_parents, by_child_pl)
flip_parents <- flip_parents %>% mutate(flip_perc = n/no_items)

flip_parents %>% ggplot(aes(x = age_CAT, y = flip_perc)) + geom_jitter() + labs(title = "child's age does not relate to amount of flips by parent (PL)", y = "flips (% of all items administered)") + ylim(0,1)

flip_parents %>% filter(duration_cat < 2500) %>% ggplot(aes(x = duration_cat, y = flip_perc)) + geom_jitter() + labs(title = "duration of CAT (in secs) does not relate to amount of flips by parent  (PL)", y = "count of flips") + ylim(0,1)

flip_parents  %>% filter(duration_full < 25000) %>% ggplot(aes(x = duration_full, y = flip_perc)) + geom_jitter() + labs(title = "duration of full (in secs) does not relate to amount of flips by parent  (PL)",  y = "flips (% of all items administered)") + ylim(0,1)

flip_parents  %>% ggplot(aes(x = score_full, y = flip_perc)) + geom_jitter() + labs(title = "full score does not relate to amount of flips by parent  (PL)", y = "flips (% of all items administered)") + ylim(0,1)

flip_parents  %>% ggplot(aes(x = abs(full_cat_diff), y = flip_perc)) + geom_jitter() + labs(title = "full-CAT diff relates to amount of flips by parent? (PL)",  y = "flips (% of all items administered)") + ylim(0,1)

flip_parents <- flip_parents %>% mutate(abs_full_cat_diff = abs(full_cat_diff))
cor.test(flip_parents$abs_full_cat_diff, flip_parents$flip_perc)
...
```{r}
# Difference between ability from full vs ability from CAT - is non-linear fit is better than linear?
# no
lin <- lm(catTheta ~ fullTheta, data = prod_s)
lin %>% ggplot(aes(x = fullTheta, y = residuals(lin))) + geom_point() + geom_smooth(method = "lm")

nonlin <- lm(catTheta ~ poly(fullTheta, 2), data = prod_s)
...
```{r d-by-lexical-class-fig, fig.width = 10, fig.height = 5, fig.cap = "The relative positioning of the items by lexical class (colored) plotted in the context of the full item pool (grey): (A) English, (B) Polish.", fig.pos = '!h', include = T}
# lexical class
...
```
```{r d_by_lexical_class_tab, include = T}
...
responses_cat_pl %>%
     select(definition, q_id) %>%
     left_join(low_a1_items_pl, join_by(definition == item)) %>% filter(!is.na(a1)) -> low_a1_items_pl
...
responses_cat_pl %>% select(definition, unlikely_item) %>% filter(unlikely_item == "yes") %>% unique() %>% summarise(n()) # 12 --> ok
...
# plot
responses_cat_pl %>%
  semi_join(ids_with_unlikely, by = "id") %>% 
...
```{r}
# Phil Chalmers' response to ^
# "High slope items are administered in CATs primarily because they create higher item information peaks around specific theta locations, but do not do so everywhere. It also depends on the difficulty of the item as well, which is a function of the intercept parameter(s). It may be better to look at the Fisher information functions instead as that function is one of the driving forces for item selection. From this perspective I would agree that the CAT is running out of items to pick so it starts selecting the most informative items around the current theta estimate, which may provide more information that items that have more peaked information curves but are concentrated around theta values that are much further away."
...
In both CAT-CDIs, we have been able to reliably estimate lexical ability for majority of children. Taking as the cut-off Makransky's suggestion to consider a SEM below 0.2 as acceptable, so defined reliable CAT estimate was gathered for `r round(sum(by_child_pl$cat_theta_se < 0.2)/count(by_child_pl)*100,1)` of the American participants, and `r round(sum(prod_s$catTheta_SE < 0.2)/count(prod_s)*100,1)` of the Polish participants. The remaining participants, i.e. those with higher standard errors of measurement (in other words, lower reliability of ability estimates) were typically those at the extremes of the ability scale--children with either relatively low or relatively high lexical skills (see Figure \@ref(fig:se-fig)).


```{r se-fig, fig.width = 10, fig.height = 5, fig.cap = "Standard Error of Measurement (SEM) by ability estimate in (A) English and (B) Polish. The red line marks the SE of 0.2.", fig.pos = '!h', include = T}
...
fig3a <- prod_s %>%
  ggplot(aes(x = catTheta, y = catTheta_SE)) + geom_point(color = "#00416d", alpha = 0.5) + geom_hline(aes(yintercept = 0.2), color = "red") +
