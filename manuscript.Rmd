...
abstract: |
  **Purpose**. The MacArthur-Bates Communicative Development Inventories (CDIs) are widely used parent-report tools for assessing children’s early vocabulary and language development. Although CDIs provide detailed insights across a wide range of lexical and semantic categories, their length can make them impractical for screening or clinical use. Recent efforts to shorten CDIs have applied Item Response Theory (IRT) and Computerized Adaptive Testing (CAT). 
  
  **Method**.   We evaluate the performance, validity, and reliability of two computerized adaptive versions of the CDIs (CAT-CDIs): one in American English and one in Polish. The English validation data, previously reported in Kachergis et al. (2022), are re-analysed here with a new comparative focus, while the Polish validation data are presented for the first time.
  
  **Results**. CAT-CDIs substantially reduced administration time compared to full CDIs. CAT-CDI estimates showed strong concurrent validity with full CDI scores and full CDI ability estimates. Reliability was excellent for both CAT-CDIs, with most children receiving precise estimates. Some systematic differences emerged: CAT estimates tended to be higher than full CDI estimates at low ability levels and more aligned at higher levels, likely reflecting differences in response behavior and item coverage. CAT-CDIs showed minimal item overuse. Cross-linguistic comparisons showed expected differences in absolute difficulty -- driven by different calibration samples -- but moderate correlations for shared items, especially within semantic categories known to show cross-linguistic stability (e.g., nouns, sound-related words).
  
  **Conslusions**. Both CAT-CDIs provide efficient, valid, and reliable assessments of early vocabulary while dramatically reducing administration time. Our data illustrate how different design choices -- such as calibration sample composition and stopping criteria -- can influence test outcomes, and how these insights can guide future CAT-CDI adaptations in selecting approaches best suited to their research or clinical goals.
...
There are 679 items in the English CAT-CDI and 666 items in the Polish CAT-CDI. Of these, `r nrow(items_en_pl)` are translation equivalents, i.e. items whose meaning is common across the two languages (e.g. "cat", "kot"). The translation equivalents were identified by using "universal lemmas", or "unilemmas", i.e., cross-linguistic conceptual mappings that link words with broadly similar meanings across languages (constructed and verified using Wordbank glosses) to enable systematic comparison of CDI items [@kachergisMeasuringChildrensEarly2025].
...
The Polish validation data is available Open Science Framework (OSF): <https://osf.io/k3bq2/>. For details on American English validation data, please see @kachergisOnlineComputerizedAdaptive2022.

# **Funding**

The Polish validation data collection and analyses were funded by the National Science Center: grant no. 2018/31/B/HS6/03916) granted to Ewa Haman and grant no. 2022/47/D/HS2/00392 granted to Karolina Muszyńska. Partial funding was also obtained from IDUB (Excellence Initiative Research University) within Action IV.2.1. "Strengthening of the research potential of the scientific staff, in particular young scientific staff, with the purpose of achieving scientific excellence"; and Action II 3.7 "Multilingualism" from which Magdalena Krysztofiak was funded.
