...
library(ggpubr)
c1 <- by_child_pl %>% 
  ggplot(aes(x = score_full, y = cat_theta)) + geom_jitter(aes(color=sex), alpha = 0.7) +
  geom_smooth(aes(color=sex), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
     xlim(0,700) + ylim(-3, 3) +
  theme_light() +
  stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  labs(x = "Score on full CDI", y = "Ability from CAT-CDI", title = "POLISH") + theme(legend.position = "bottom")

c2 <- prod_s %>% 
  ggplot(aes(x = production, y = catTheta)) + geom_jitter(aes(color=sex_full), alpha = 0.7) +
  geom_smooth(aes(color=sex_full), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
     xlim(0,700) + ylim(-3, 3) +
  theme_light() + theme(legend.title = element_blank()) +
  # stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  annotate("text", x=200, y=2.65, label=paste("italic(r)==", "0.86*','~italic(p)<", "0.001"), size = 5, parse = T) + 
  labs(x = "Score on full CDI", y = "Ability from CAT-CDI", title = "ENGLISH") + theme(legend.position = "bottom")

cor_plot <- ggarrange(c2, c1, ncol = 2, common.legend = T, legend = "bottom")
cor_plot
```

```{r}
# density plot
ggplot() + geom_density(data = by_child_pl, aes(x = cat_theta), color = "blue") +
          geom_vline(xintercept = median(by_child_pl$cat_theta), color = "blue", alpha = 0.5) +
     geom_density(data = prod_s, aes(x = catTheta), color = "green") +
     geom_vline(xintercept = median(prod_s$catTheta), color = "green", alpha = 0.5) +
     labs(x = "Ability from CAT", title = "Blue is Polish, Green is English") +
     theme_light()
```
...
c3 <- by_child_pl %>% 
  ggplot(aes(x = full_theta, y = cat_theta)) + geom_jitter(aes(color=sex), alpha = 0.7) +
  geom_smooth(aes(color=sex), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
     xlim(-3, 3) + ylim(-3, 3) +
  theme_light() +
  stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  labs(x = "Ability from full CDI", y = "Ability from CAT-CDI", title = "POLISH") + theme(legend.position = "bottom")

c4 <- prod_s %>% 
  ggplot(aes(x = fullTheta, y = catTheta)) + geom_jitter(aes(color=sex_full), alpha = 0.7) +
  geom_smooth(aes(color=sex_full), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
     xlim(-3, 3) + ylim(-3, 3) +
  theme_light() + theme(legend.title = element_blank()) +
  # stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  annotate("text", x=-1.3, y=2.65, label=paste("italic(r)==", "0.92*','~italic(p)<", "0.001"), size = 5, parse = T) + 
  labs(x = "Ability from full CDI", y = "Ability from CAT-CDI", title = "ENGLISH") + theme(legend.position = "bottom")

cor_plot2 <- ggarrange(c4, c3, ncol = 2, common.legend = T, legend = "bottom")
cor_plot2

# mark extreme discrepancy cases
c5 <- by_child_pl %>% 
  ggplot(aes(x = full_theta, y = cat_theta)) + geom_jitter(aes(color=sex), alpha = 0.7) +
  geom_smooth(aes(color=sex), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
  geom_jitter(data = subset(by_child_pl, extreme_discrep == "yes"), color = "red", size = 2, alpha = 0.9) +
     xlim(-3, 3) + ylim(-3, 3) +
  theme_light() +
  stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  labs(x = "Ability from full CDI", y = "Ability from CAT-CDI", title = "POLISH") + theme(legend.position = "bottom")
...
c6 <- prod_s %>% 
  ggplot(aes(x = fullTheta, y = catTheta)) + geom_jitter(aes(color=sex_full), alpha = 0.7) +
  geom_smooth(aes(color=sex_full), method = "lm", fill = "#a7a7a7", alpha = 0.2) +
  geom_jitter(data = subset(prod_s, extreme_discrep == "yes"), color = "red", size = 2, alpha = 0.9) +
     xlim(-3, 3) + ylim(-3, 3) +
  theme_light() + theme(legend.title = element_blank()) +
  # stat_cor(cor.coef.name = "r", p.accuracy = 0.001, r.accuracy = 0.01, size = 5, show.legend = F) +
  annotate("text", x=-1.3, y=2.65, label=paste("italic(r)==", "0.92*','~italic(p)<", "0.001"), size = 5, parse = T) + 
...
cor_plot3 <- ggarrange(c6, c5, ncol = 2, common.legend = T, legend = "bottom")
cor_plot3
...
items_cat_pl <- merge(items_cat_pl, items_full_pl[,c("item", "lexical_class")], all.x = T, 
                      by.x = "item", by.y = "defininion") 
...
# PL
by_child_pl %>% summarise(mean(no_items),
                          median(no_items),
                          min(no_items),
                          max(no_items),
                          sd(no_items))
