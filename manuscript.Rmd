...
We also calculated Cohen's kappa per each parent, considering their observed agreement (between responses to the same items in full and CAT CDI) and expected agreement by chance. The kappa values indicated fair (or close to fair) agreement: English *M* =  `r round(mean(kappa_results_en$kappa, na.rm = T),2) #0.4`, *Mdn* = `r round(median(kappa_results_en$kappa, na.rm = T),2)`, Polish *M* =  `r round(mean(kappa_results_pl$kappa, na.rm = T),2) #0.4`, *Mdn* = `r round(median(kappa_results_pl$kappa, na.rm = T),2)`.
