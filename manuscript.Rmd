...
Notably, the two validation samples differed slightly (but significantly) in mean age, `r t_age$full_result`, which may affect the outcomes of direct comparisons between them, as age could be a contributing factor. We present one such comparison--standard error of CAT ability--where this difference is explicitly indicated. *NOTE*: You can also view/comment on this on GitHub [Issue #7 linked here](https://github.com/karolina-m/catcdi_validation_eng_pl/issues/7).
...
<!-- I think it would be good to explain here in 3-4 sentences how a CAT CDI works: how we obtain the initial theta estimate, show the parent one item at a time, when do we stop the test and that in the end, we get a theta and theta's error. Something to allow the reader to imagine the process of CAT-CDI testing. -->
...
```
*Is CAT ability always higher than full?*
Not *always* but for large majority of kids.
```{r}
prod_s %>% ggplot(aes(x = full_cat_diff)) + geom_histogram() + labs(title = "Mostly, CAT ability higher than full ability")
```
*Are parents responding "yes, produces" too often?*
```{r}
# PL
yes_pl <- responses_cat_pl %>%
     filter(id %in% by_child_pl$id) %>%
     group_by(id) %>%
     mutate(n_yes = sum(answers),
            n_asked = max(q_id)) %>% select(id, n_asked, n_yes) %>% unique() %>%
     mutate(perc_yes = n_yes/n_asked) 
yes_pl %>%
     ggplot(aes(x = perc_yes)) + geom_histogram() + labs(title = "Percent of responses <<yes>> by parent - PL")


# EN
yes_en <- resps %>%
     filter(response_cat != "no_test") %>%
     group_by(subject_id) %>%
     mutate(n_yes = sum(response_cat == "produces", na.rm = TRUE),
            n_asked = n()) %>% select(subject_id, n_yes, n_asked) %>% unique() %>%
     mutate(perc_yes = n_yes/n_asked) 
yes_en %>%
     ggplot(aes(x = perc_yes)) + geom_histogram() + labs(title = "Percent of responses <<yes>> by parent - EN")
     
```
*Is perc of "yes" linked to SE and/or the difference in abilities from CAT and full?*
```{r}
# PL
by_child_pl <- merge(by_child_pl, yes_pl, all.x = T, by = "id")

by_child_pl %>% ggplot(aes(x = perc_yes, y = cat_theta_se)) + geom_point()
by_child_pl %>% ggplot(aes(x = perc_yes, y = abs(full_cat_diff))) + geom_point()


# EN
prod_s <- merge(prod_s, yes_en, all.x = T, by = "subject_id")

prod_s %>% ggplot(aes(x = perc_yes, y= catTheta_SE)) + geom_point()
prod_s %>% ggplot(aes(x = perc_yes, y= abs(full_cat_diff))) + geom_point()

```


*Is CAT generally less precise than full ability?*
Yes, but isn't that to be expected? Ability form full is more informed (all items are being used to estimate ability).
```{r}
ggplot(prod_s) +
  geom_density(aes(x = catTheta_SE), fill = "blue", alpha = 0.4) +
  geom_density(aes(x = fullTheta_SE), fill = "green", alpha = 0.4) +
          xlim(0,0.6) +
  labs(title = "Distribution of Standard Errors in ENGLISH",
       x = "Standard Error", y = "Density")

# PL
ggplot(by_child_pl) +
  geom_density(aes(x = cat_theta_se), fill = "blue", alpha = 0.4) +
  geom_density(aes(x = full_theta_se), fill = "green", alpha = 0.4) +
     xlim(0,0.6) +
  labs(title = "Distribution of Standard Errors in POLISH",
       x = "Standard Error", y = "Density")
```
```{r}
ggplot(prod_s, aes(x = catTheta_SE, y = abs(sq_err))) +
  geom_point(alpha = 0.5) +
  geom_smooth(method = "loess", color = "red") +
  labs(title = "CAT SE is not higher for larger disrepancies",
       x = "CAT Standard Error", y = "Absolute Error (|CAT − Full|)")
```


Another possibility for high MSE in the English dataset is that CAT-CDI might be less precise for certain ability levels, thus increasing the discrepancy between

the difference between ability estimates from CAT-CDI and full CDI is higher for very low or very high levels of ability.

```{r}
# Higher variability at ends of ability scale even after removing extreme cases?

# EN
prod_s %>% filter(extreme_discrep == "no") %>% 
     ggplot(aes(x = fullTheta, y = abs(sq_err))) + geom_point() + geom_smooth(method = "loess") + 
     labs(title = "ENGLISH", y = "Squared Error (CAT vs full ability) (abs)")
...
```{r}
# non-linear fit is better than linear?
# no
