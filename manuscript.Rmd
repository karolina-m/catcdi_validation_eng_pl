...
We found similarly strong correlations in the two languages between the abilities estimated from CDI-CAT and full CDI scores (American English and Polish: $r$ = .86). We also obtained ability estimates from the full CDIs, which is a robust measure of ability because it is based on all available item responses, maximizing precision. Such ability from the full test version is often considered a baseline. We then correlated the abilities estimated from the CDI-CAT and abilities estimated from full CDI and against found strong correlations in both languages, English and Polish: $r$ = .92. Finally, we also correlated the abilities estimated from the full CDI and the full CDI scores, i.e. two measures obtained from the same data. This can be done to examine consistency between model-based and traditional scoring methods. Again, we found strong correlations in both languages (American English: $r$ = .95, Polish: $r$ = `r cor(by_child_pl$full_theta, by_child_pl$score_full)`), confirming that both measures reflect the same underlying ability. The abilities estimated from the CDI-CAT and the full CDI scores were also strongly correlated within individual age groups (see Table \@ref(tab:corr-by-age)).
